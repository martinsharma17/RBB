...
function KycFormMaster1({
  ...
  const [occupationTypes, setOccupationTypes] = useState<string[]>([]);
  ...
  // Get API base from Vite env
  const apiBase = import.meta.env.VITE_API_URL;
  const token = localStorage.getItem("authToken");

  useEffect(() => {
    // Only fetch sessionId if not already provided by prop
    if (sessionId || !token || !apiBase) {
      if (!token || !apiBase) {
        console.warn(
          "Token or API base URL not set. SessionId not initialized."
        );
      }
      return;
    }

    apiFetch(`${apiBase}/api/Kyc/my-session`)
      .then((res) => {
        if (!res.ok) {
          console.error("Failed to fetch sessionId: response not ok");
          return;
        }
        return res.json();
      })
  ...
  useEffect(() => {
    async function fetchOccupationTypes() {
      try {
        const res = await fetch(`${apiBase}/api/Occupation`, {
        ...
        if (Array.isArray(data)) {
          setOccupationTypes([
            ...data.map((o) => o.name || o.title || o),
            "Other",
          ]);
        }
      } catch (err) {
        console.error("Failed to fetch occupation types:", err);
      }
    }
  ...
  const handleNext = async () => {
    ...
    if (endpoint) {
      ...
      console.log("KYC sessionId:", sessionId);
      // Convert sessionId to integer if possible
      if (sessionId && !isNaN(Number(sessionId))) {
        payload.sessionId = Number(sessionId);
      } else {
        alert(
          "Session ID is missing or invalid. Please refresh the page or restart the process."
        );
        return;
      }

      // Add other step fields to payload as needed
      // Example: payload.firstName = values.firstName; etc.

      const finalPayload = { model: payload };
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      await fetch(`/api/kyc/submit/${sessionIdFromStorage}`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
            ...
            <Controller
              ...
              render={({ field }) => (
                <select {...field} className="form-input">
                  ...
                  <option value="Father">
                    {t("step6.relationship.father")}
                  </option>
                  <option value="Mother">
                    {t("step6.relationship.mother")}
                  </option>
