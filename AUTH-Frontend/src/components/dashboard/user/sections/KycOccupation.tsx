...
const KycOccupation: React.FC<KycOccupationProps> = ({
  ...
  useEffect(() => {
    ...
    fetch(`${baseUrl}/api/Occupation`, fetchOptions)
      .then((res) => {
      ...
      .catch((err) => {
        console.error("[Occupation] Fetch failed:", err);
        setOccupationOptions([
          { value: "Salaried", label: "Salaried" },
          { value: "Self Employed", label: "Self Employed" },
          { value: "Business", label: "Business" },
    ...
    <form onSubmit={(e) => handleSubmit(e, false)} className="space-y-8 animate-fadeIn">
      <header className="border-b border-indigo-100 pb-5">
        <h2 className="text-2xl font-extrabold text-indigo-900 flex items-center gap-2">
          <span className="w-8 h-8 bg-indigo-600 text-white rounded-full flex items-center justify-center text-sm">5</span>
          Occupation Details
        </h2>
        <p className="text-gray-500 mt-1 ml-10">
          Please provide accurate information regarding your current profession and income sources.
        </p>
      </header>
      ...
      <fieldset className="space-y-6">
        <legend className="text-sm font-bold text-indigo-600 uppercase tracking-wider mb-4 px-1">Professional Identity</legend>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 p-1">
          ...
          {formData.occupation === "Others" && (
            <section className="flex flex-col space-y-1.5 animate-slideDown">
              <label htmlFor="otherOccupation" className="text-sm font-semibold text-gray-700">
      ...
      <fieldset className="space-y-6 pt-2">
        <legend className="text-sm font-bold text-indigo-600 uppercase tracking-wider mb-4 px-1">Workplace Information</legend>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 p-1">
          <section className="flex flex-col space-y-1.5">
            <label htmlFor="orgName" className="text-sm font-semibold text-gray-700">
              Organization Name
            </label>
            <input
              id="orgName"
          ...
          <section className="flex flex-col space-y-1.5">
            <label htmlFor="employeeIdNo" className="text-sm font-semibold text-gray-700">
          ...
          <section className="flex flex-col col-span-full space-y-1.5">
            <label htmlFor="orgAddress" className="text-sm font-semibold text-gray-700">
          ...
          <section className="flex flex-col space-y-1.5">
            <label htmlFor="serviceSector" className="text-sm font-semibold text-gray-700">
              Service Sector
            </label>
            <select
              id="serviceSector"
      ...
      <fieldset className="space-y-6 pt-2">
        <legend className="text-sm font-bold text-indigo-600 uppercase tracking-wider mb-4 px-1">Financial Data</legend>
        ...
        <div className="bg-gradient-to-br from-gray-50 to-indigo-50/30 p-6 rounded-2xl border border-indigo-100 shadow-sm">
          ...
          <div className="relative pt-2">
            <input
              ...
              step="1"
              value={
                formData.annualIncomeBracket === "Below1Lakh" ? "0" :
                  formData.annualIncomeBracket === "1LakhTo5Lakh" ? "1" :
                    formData.annualIncomeBracket === "5LakhTo10Lakh" ? "2" :
                      formData.annualIncomeBracket === "10LakhTo20Lakh" ? "3" :
                        formData.annualIncomeBracket === "Above20Lakh" ? "4" : "0"
              }
              onChange={(e) => {
                ...
                setFormData(prev => ({ ...prev, annualIncomeBracket: map[val] }));
              }}
            ...
            <div className="flex justify-between text-[11px] text-gray-500 font-bold px-1 mt-3">
              <span className="flex flex-col items-center"><span>|</span><span>Below 1L</span></span>
              <span className="flex flex-col items-center"><span>|</span><span>1L - 5L</span></span>
              <span className="flex flex-col items-center"><span>|</span><span>5L - 10L</span></span>
              <span className="flex flex-col items-center"><span>|</span><span>10L - 20L</span></span>
              <span className="flex flex-col items-center"><span>|</span><span>Above 20L</span></span>
            </div>
          </div>
        </div>
      </fieldset>
